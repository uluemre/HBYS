...
const RegisterForm = ({ yeniHastaEkle, hataMesaji, onGeriDon }) => {
    ...
    const handleKayitOlSubmit = async () => {
        ...
        if (!validateTC(kayitTc)) {
            setKayitHata("Geçersiz T.C. Kimlik Numarası!");
            return;
        }
        if (kayitSifre.length < 5 || !/[A-Z]/.test(kayitSifre) || !kayitSifre.includes('.')) {
            setKayitHata("Şifre: En az 5 hane, 1 Büyük Harf ve Nokta (.) içermeli!");
            return;
        }
        await yeniHastaEkle({
            adSoyad: kayitAd,
        ...
        <div className="login-page-wrapper">
            <div className="login-card" style={{ maxWidth: '500px' }}>
                ...
                <div className="login-form">
                    ...
                    <button type="button" className="back-link-btn" onClick={onGeriDon}>← Giriş Ekranına Dön</button>
                </div>
            </div>
        </div>
    );
